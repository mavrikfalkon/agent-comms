...
| This Grok TUI | Project MCP `agent-comms` in `.grok/config.toml` → `Start-AgentCommsMcp.cmd`. |
| ChatGPT / Codex | `~\.codex\config.toml` `[mcp_servers.agent-comms]` → same launcher. Restart the ChatGPT app. |
